...
df = pd.read_csv('./data_choshun/matome_choshun.csv', encoding='shift-jis',
                 usecols=['発生日', '発生時刻', '射出最前進位置[mm]', 'V-P切換圧[MPa]', '射出最前進圧[MPa]', '射出ピーク圧[MPa]'])


today_df = df.query('発生日 >= "2023/01/01"')
...
select_df['射出最前進位置[mm]しきい値'] = 5.95
select_df['V-P切換圧[MPa]しきい値'] = 75.0
...
line.x_axis.title = '射出最前進位置のしきい値は「5.95mm」　これ以上はショートリスク有り！'
...
line2.x_axis.title = 'VP切替圧のしきい値は「75.0Mpa」　これ以下はショートリスク有り！'


ws.add_chart(line2, 'A40')


wb.save('H:\マイドライブ\成形機モニターデータ\長春モニターデータ\長春モニターデータ.xlsx')
